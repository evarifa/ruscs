...
import pytest
from ruscs.mt_density_dyn_pulse_choice_qs import *
...
class TestMTDensityDynQS:
    ...
    def test_negative_rate(self):
        ...
        kmax = 2
        maxiter = 10
        new_connections = 0  # Example new connections
        with pytest.raises(ValueError, match="Parameter rate must be non-negative."):
            mt_density_dynamic_qs_pulse_choice(
    ...
    def test_new_connections(self):
        """Test that a ValueError is raised when new_connections is not of the correct format."""
        # Initialize network for test
        neighbours = [[1], [3, 2], [1]]
        kmax = 2
        maxiter = 10
        infected_vector = np.zeros(maxiter)
        infected_vector[1] = 1  # Infect the first node at the start
        new_connections = [[1,2]]
        
        result = mt_density_dynamic_qs_pulse_choice(
            infected_vector = infected_vector,
            rate = 0.1,
            alpha = 0.5,
            delta = 0.5,
            maxiter = maxiter,
